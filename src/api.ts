...
import { SeverityEnum, GroupedPath, ShareRoot, PostTempShareResponse, FilelistItem, Extension, Bundle } from './types/api';
...
export const API = (socket: APISocket) => {
  ...
  const getExtensions = (): Promise<Extension[]> => {
    return socket.get('extensions');
  };

  const disableExtension = (id: string): Promise<void> => {
    return socket.patch(`extensions/${id}`, { disabled: true });
  };
  ...
  const getQueueBundle = (token: number) => {
    return socket.get<Bundle>(`queue/bundles/${token}`);
  };

  const shareBundle = (token: number, skipValidation: boolean) => {
    return socket.post(`queue/bundles/${token}/share`, {
